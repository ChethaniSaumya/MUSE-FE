...
const Home = () => {
	...
	const [isFreshVisit, setIsFreshVisit] = useState(true);
	...
	useEffect(() => {
		...
		setIsFreshVisit(navigationType === 'navigate' || navigationType === 'reload');
	...
	useEffect(() => {
		if (_connected && userMintedCount !== undefined) {
			...
			if (Number(userMintedCount) === 0) {
				...
				setGlowMintButton((hasExistingName || name) && (hasExistingEmail || email));
			}
		}
	...
	useEffect(() => {
		// Only show arrow if:
		// 1. User hasn't connected before
		// 2. Not currently connected
		// 3. Not in the middle of any process
		...
		setShowConnectArrow(shouldShowArrow);
	}, [
		hasConnectedBefore,
		_connected,
		isMinting,
		...
		<div className="app-container">
					...
					{_connected ? (
						...
						<div className="connect-button-container">
							<button
								className="connect-wallet-btn"
								onClick={() => {
									setShowConnectArrow(false);
									open();
								}}
								onMouseEnter={() => !_connected && setShowConnectArrow(true)}
								onMouseLeave={() => !_connected && setShowConnectArrow(false)}
								disabled={isMinting || isDownloading || isCertificatePreparing}
							>
			...
			<main className="main-content">
				...
				<section className="mint-section">
					...
					{/* Terms and Conditions Section - Only show for first mint */}
					{isFirstMint() && (
						<div className="terms-section">
							...
							<div className="terms-text">
								<p>By minting this NFT, you agree to our <a href="/terms" target="_blank" rel="noopener noreferrer">Terms of Service</a> and <a href="/privacy" target="_blank" rel="noopener noreferrer">Privacy Policy</a>.</p>
							</div>
						</div>
					)}

					<button
						className={`mint-btn ${glowMintButton ? 'glow' : ''}`}
						onClick={mintNFT}
						disabled={isMintDisabled || isCertificatePreparing || !glowMintButton}
					>
						{isMinting
							? 'Minting...'
							: !_connected
